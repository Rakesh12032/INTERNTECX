...
async function sendMail({ to, subject, html }) {
  ...
  return transporter.sendMail({
    from: `"Interntex" <${process.env.EMAIL_USER}>`,
    to,
    subject,
    html
  });
  ...
  return sendMail({
    ...
    html: `
      <div style="font-family:Inter,Arial,sans-serif;padding:24px;background:#f8fafc;color:#0f172a">
        ...
        <p>Your Interntex verification code is:</p>
...
export async function sendWelcomeEmail(email, name) {
  return sendMail({
    to: email,
    subject: "Welcome to Interntex",
    html: `<div style="font-family:Inter,Arial,sans-serif;padding:24px;"><h2>Welcome, ${name}!</h2><p>Your Interntex account is now active. Learn. Intern. Succeed.</p></div>`
  });
}
...
export async function sendCertificateEmail(email, name, courseName, certId) {
  return sendMail({
    to: email,
    subject: "Your Interntex Certificate",
...
export async function sendWithdrawalEmail(email, name, amount, status) {
  return sendMail({
    to: email,
    subject: `Withdrawal ${status} - Interntex`,
...
export async function sendWeeklyReport(email, name, stats) {
  return sendMail({
    to: email,
    subject: "Your Weekly Interntex Progress Report",
